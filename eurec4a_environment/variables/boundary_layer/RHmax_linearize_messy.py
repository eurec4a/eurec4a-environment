...
def calc_peakRH_linearize(ds, altitude="height", rh="rh", time_dim="sounding", z_min=200.0, z_max=900.0, z_min_lin=50.0, z_max_lin=400.0):
     """
     ...
     mixed_layer = np.logical_and(ds[altitude] >= z_min, ds[altitude] <= 1500)  # enforce z_max later
     ml_linfit = np.logical_and(ds[altitude] >= z_min_lin, ds[altitude] <= z_max_lin) # for linearized RH profile
     ...
     for i in range(len(ds[rh])):
         ...
         model = sm.OLS(rh_profile[ml_linfit].values,X).fit() # instantiate linear model 
         ...
         linearized_RH_values = linearized_RH[idx_peaks_RH] # linearized RH values at the observed RH maxima
         min_diff_RH = min(abs(obs_RH_peaks - linearized_RH_values)) # find minimum RH difference between observed peaks and linearized profile
...
z_peakRH_linfit = calc_peakRH_linearize(all_sondes, altitude="height", rh="rh", time_dim="sounding", z_min=200.0, z_max=900.0, z_min_lin = 50.0, z_max_lin=200.0)     
